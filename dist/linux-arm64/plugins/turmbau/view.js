...
(function () {
    ...
    if (host.dataset.profile !== profileId || !host.querySelector('svg')) {
      ...
      try {
        ...
        if (svg) {
          ...
          svg.classList.add('tw-scheibe-svg');
        }
      } catch (e) {
        host.innerHTML = '<div class="tw-muted">Scheibe nicht ladbar</div>';
        return;
      }
    }
    const svg = host.querySelector('svg');
    if (!svg) return;
    let g = svg.querySelector('#tw-shots');
    if (!g) {
      g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('id', 'tw-shots');
      svg.appendChild(g);
    }
  ...
  function recentHtml(game, focusRange) {
    ...
    return '<div class="tw-panel"><h3>Letzte Schüsse</h3><ul class="tw-recent">' + list.map(function (s) {
      const me = focusRange && Number(s.rangeNum) === Number(focusRange);
      return '<li class="' + (me ? 'is-me' : '') + '"><span class="tw-swatch" style="background:' +
  ...
  function standingsHtml(game, focusRange) {
    ...
    return '<ul class="tw-standings">' + list.map(function (p) {
  ...
  function shotBlock(title, shot) {
    if (!shot) return '<div class="tw-panel"><h3>' + esc(title) + '</h3><div class="tw-muted">—</div></div>';
    return '<div class="tw-panel"><h3>' + esc(title) + '</h3><div class="tw-shot-val">' +
      fmt1(shot.raw) + '</div><div class="tw-meta">' + esc(shot.note || '') + '</div></div>';
  }
  ...
  function setTwSurfaceClasses(container, mode) {
    if (!container) return;
    container.classList.add('range-plugin-view', 'tw-view');
    if (container.id === 'shared-master-host') {
      container.classList.add('shared-master-host');
    }
    if (mode === 'master') {
      container.classList.add('tw-race-master');
      container.classList.remove('tw-race-shooter');
    } else {
      container.classList.add('tw-race-shooter');
      container.classList.remove('tw-race-master');
    }
  }

  function beginPaint(container) {
    const gen = (container._twPaintGen || 0) + 1;
    container._twPaintGen = gen;
    return gen;
  }

  function twPaintStale(container, gen) {
    return !container || container._twPaintGen !== gen;
  }
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    if (container.querySelector('.' + otherCls) || !container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        ...
        '</div>' +
        '<div class="tw-board-col">' +
        '<div class="tw-board-host" data-board></div>' +
        '</div></div>' +
        '<footer class="tw-footer" data-footer></footer></div>';
    }
    ...
    if (!header) return;
    header.innerHTML =
      '<span class="tw-title">Turmbau</span>' +
      '<span class="tw-badge">' + esc(PHASE_LABEL[game.phase] || game.phase || '') + '</span>' +
      (isShooter
        ? '<span class="tw-meta">Stand ' + esc(focusRange) +
    ...
    if (footer) {
      footer.innerHTML = '<span class="tw-meta">Teiler neigt den Turm · über 8,5 höher stapeln · Hole-in-Hole über 8,5 stabilisiert</span>';
    }
    if (container.id === 'shared-master-host') {
