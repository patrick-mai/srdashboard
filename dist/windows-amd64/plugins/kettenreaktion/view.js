...
(function () {
  const PLUGIN_ID = 'kettenreaktion';
  let audioCtx = null;
  let lastEventSig = '';
    ...
    if (host.dataset.profile !== profileId || !host.querySelector('svg')) {
      ...
      try {
        ...
        if (svg) {
          ...
          svg.classList.add('kr-scheibe-svg');
        }
      } catch (e) {
        host.innerHTML = '<div class="kr-muted">Scheibe nicht ladbar</div>';
        return;
      }
    }
    const svg = host.querySelector('svg');
    if (!svg) return;
    let g = svg.querySelector('#kr-shots');
    if (!g) {
      g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('id', 'kr-shots');
      svg.appendChild(g);
    }
  ...
  function recentHtml(game, focusRange) {
    ...
    return '<div class="kr-panel"><h3>Letzte Schüsse</h3><ul class="kr-recent">' + list.map(function (s) {
      const me = focusRange && Number(s.rangeNum) === Number(focusRange);
      return '<li class="' + (me ? 'is-me' : '') + '"><span class="kr-swatch" style="background:' +
  ...
  function standingsHtml(game, focusRange) {
    ...
    return '<ul class="kr-standings">' + list.map(function (p) {
  ...
  function shotBlock(title, shot) {
    if (!shot) return '<div class="kr-panel"><h3>' + esc(title) + '</h3><div class="kr-muted">—</div></div>';
    return '<div class="kr-panel"><h3>' + esc(title) + '</h3><div class="kr-shot-val">' +
      fmt1(shot.raw) + '</div><div class="kr-meta">' + esc(shot.note || '') + '</div></div>';
  }
  ...
  function boardSvg(game, focusRange) {
    ...
    if (!players.length) return '<div class="kr-muted">Keine Kette</div>';
    return '<div class="kr-cards kr-cards-' + cardsCols(players.length) + '">' + players.map(function (pl) {
      const n = Math.max(0, Number(pl.chainLen) || 0);
      let dots = '';
      for (let i = 0; i < 8; i++) {
        const on = i < n;
        dots += '<circle cx="' + (30 + i * 28) + '" cy="22" r="10" fill="' + (on ? esc(pl.color || '#7ec8ff') : 'rgba(255,255,255,0.15)') + '"/>';
      }
      const mine = Number(pl.rangeNum) === Number(focusRange);
      return '<div class="kr-card' + (mine ? ' is-me' : '') + '">' +
        '<div class="kr-card-label">' + esc(pl.label) + ' ×' + fmt1(pl.multiplier || 1) + '</div>' +
        '<svg class="kr-card-svg" viewBox="0 0 250 44" preserveAspectRatio="xMidYMid meet">' + dots + '</svg></div>';
    }).join('') + '</div>';
  }

  function setKrSurfaceClasses(container, mode) {
    if (!container) return;
    container.classList.add('range-plugin-view', 'kr-view');
    if (container.id === 'shared-master-host') {
      container.classList.add('shared-master-host');
    }
    if (mode === 'master') {
      container.classList.add('kr-race-master');
      container.classList.remove('kr-race-shooter');
    } else {
      container.classList.add('kr-race-shooter');
      container.classList.remove('kr-race-master');
    }
  }

  function beginPaint(container) {
    const gen = (container._krPaintGen || 0) + 1;
    container._krPaintGen = gen;
    return gen;
  }

  function krPaintStale(container, gen) {
    return !container || container._krPaintGen !== gen;
  }
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    setKrSurfaceClasses(container, isShooter ? 'shooter' : 'master');

    const layoutCls = isShooter ? 'kr-shooter-layout' : (isCompact ? 'kr-compact-layout' : 'kr-master-layout');
    if (!container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        '<div class="' + layoutCls + '">' +
        '<header class="kr-header" data-header></header>' +
        '<div class="kr-main">' +
        '<div class="kr-target-col">' +
        (isCompact ? '' : '<div class="kr-scheibe-wrap" data-scheibe></div>') +
        (isShooter ? '<div class="kr-shot-hud" data-shothud></div>' : '') +
        '<div data-recent></div>' +
        '<div data-standings></div>' +
        '<div data-actions></div>' +
        '</div>' +
        '<div class="kr-board-col">' +
        '<div class="kr-board-host" data-board></div>' +
        '</div></div>' +
        '<footer class="kr-footer" data-footer></footer></div>';
    }

    await ensureTargetRegistry(assetsBase);
    if (krPaintStale(container, paintGen)) return;
    const core = window.SRCore;
    ...
    if (!header) return;
    header.innerHTML =
      '<span class="kr-title">Kettenreaktion</span>' +
      '<span class="kr-badge">' + esc(PHASE_LABEL[game.phase] || game.phase || '') + '</span>' +
      (isShooter
        ? '<span class="kr-meta">Stand ' + esc(focusRange) +
          (me && me.shooterName ? ' · ' + esc(me.shooterName) : '') + '</span>'
        : '') +
      '<span class="kr-status">' + esc(game.statusLine || '') + '</span>';
    ...
    container.querySelector('[data-standings]').innerHTML =
      '<div class="kr-panel"><h3>Stand</h3>' + standingsHtml(game, isShooter ? focusRange : null) + '</div>';
    container.querySelector('[data-board]').innerHTML =
      (game.startBlockedReason ? '<div class="kr-block">' + esc(game.startBlockedReason) + '</div>' : '') +
      boardSvg(game, isShooter ? focusRange : null);

    const footer = container.querySelector('[data-footer]');
    if (footer) {
      footer.innerHTML = '<span class="kr-meta">Kette nur mit Hole-in-Hole über 8,5 · Bruch setzt zurück · höchste Punkte gewinnen</span>';
    }
    if (container.id === 'shared-master-host') {
