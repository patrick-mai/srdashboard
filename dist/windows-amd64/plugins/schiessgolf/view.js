...
(function () {
    ...
    if (host.dataset.profile !== profileId || !host.querySelector('svg')) {
      ...
      try {
        ...
        if (svg) {
          ...
          svg.classList.add('sg-scheibe-svg');
        }
      } catch (e) {
        host.innerHTML = '<div class="sg-muted">Scheibe nicht ladbar</div>';
        return;
      }
    }
    const svg = host.querySelector('svg');
    if (!svg) return;
    let g = svg.querySelector('#sg-shots');
    if (!g) {
      g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('id', 'sg-shots');
      svg.appendChild(g);
    }
  ...
  function recentHtml(game, focusRange) {
    ...
    return '<div class="sg-panel"><h3>Letzte Schüsse</h3><ul class="sg-recent">' + list.map(function (s) {
      const me = focusRange && Number(s.rangeNum) === Number(focusRange);
      return '<li class="' + (me ? 'is-me' : '') + '"><span class="sg-swatch" style="background:' +
  ...
  function standingsHtml(game, focusRange) {
    ...
    return '<ul class="sg-standings">' + list.map(function (p) {
  ...
  function shotBlock(title, shot) {
    if (!shot) return '<div class="sg-panel"><h3>' + esc(title) + '</h3><div class="sg-muted">—</div></div>';
    return '<div class="sg-panel"><h3>' + esc(title) + '</h3><div class="sg-shot-val">' +
      fmt1(shot.raw) + '</div><div class="sg-meta">' + esc(shot.note || '') + '</div></div>';
  }
  ...
  function setSgSurfaceClasses(container, mode) {
    if (!container) return;
    container.classList.add('range-plugin-view', 'sg-view');
    if (container.id === 'shared-master-host') {
      container.classList.add('shared-master-host');
    }
    if (mode === 'master') {
      container.classList.add('sg-race-master');
      container.classList.remove('sg-race-shooter');
    } else {
      container.classList.add('sg-race-shooter');
      container.classList.remove('sg-race-master');
    }
  }

  function beginPaint(container) {
    const gen = (container._sgPaintGen || 0) + 1;
    container._sgPaintGen = gen;
    return gen;
  }

  function sgPaintStale(container, gen) {
    return !container || container._sgPaintGen !== gen;
  }
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    if (!container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        ...
        '</div>' +
        '<div class="sg-board-col">' +
        '<div class="sg-board-host" data-board></div>' +
        '</div></div>' +
        '<footer class="sg-footer" data-footer></footer></div>';
    }
    ...
    if (!header) return;
    header.innerHTML =
      '<span class="sg-title">Schießgolf</span>' +
      '<span class="sg-badge">' + esc(PHASE_LABEL[game.phase] || game.phase || '') + '</span>' +
      (isShooter
        ? '<span class="sg-meta">Stand ' + esc(focusRange) +
    ...
    if (footer) {
      footer.innerHTML = '<span class="sg-meta">Wert = Weite · einlochen mit 10,5+ oder Hole-in-Hole über 8,5 · Netto gegen Vorgabe</span>';
    }
    if (container.id === 'shared-master-host') {
