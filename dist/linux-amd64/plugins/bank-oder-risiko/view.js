...
(function () {
  const PLUGIN_ID = 'bank-oder-risiko';
  let audioCtx = null;
  let lastEventSig = '';
    ...
    if (host.dataset.profile !== profileId || !host.querySelector('svg')) {
      ...
      try {
        ...
        if (svg) {
          ...
          svg.classList.add('bk-scheibe-svg');
        }
      } catch (e) {
        host.innerHTML = '<div class="bk-muted">Scheibe nicht ladbar</div>';
        return;
      }
    }
    const svg = host.querySelector('svg');
    if (!svg) return;
    let g = svg.querySelector('#bk-shots');
    if (!g) {
      g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('id', 'bk-shots');
      svg.appendChild(g);
    }
  ...
  function recentHtml(game, focusRange) {
    ...
    return '<div class="bk-panel"><h3>Letzte Schüsse</h3><ul class="bk-recent">' + list.map(function (s) {
      const me = focusRange && Number(s.rangeNum) === Number(focusRange);
      return '<li class="' + (me ? 'is-me' : '') + '"><span class="bk-swatch" style="background:' +
  ...
  function standingsHtml(game, focusRange) {
    ...
    return '<ul class="bk-standings">' + list.map(function (p) {
  ...
  function shotBlock(title, shot) {
    if (!shot) return '<div class="bk-panel"><h3>' + esc(title) + '</h3><div class="bk-muted">—</div></div>';
    return '<div class="bk-panel"><h3>' + esc(title) + '</h3><div class="bk-shot-val">' +
      fmt1(shot.raw) + '</div><div class="bk-meta">' + esc(shot.note || '') + '</div></div>';
  }
  ...
  function setBkSurfaceClasses(container, mode) {
    if (!container) return;
    container.classList.add('range-plugin-view', 'bk-view');
    if (container.id === 'shared-master-host') {
      container.classList.add('shared-master-host');
    }
    if (mode === 'master') {
      container.classList.add('bk-race-master');
      container.classList.remove('bk-race-shooter');
    } else {
      container.classList.add('bk-race-shooter');
      container.classList.remove('bk-race-master');
    }
  }

  function beginPaint(container) {
    const gen = (container._bkPaintGen || 0) + 1;
    container._bkPaintGen = gen;
    return gen;
  }

  function bkPaintStale(container, gen) {
    return !container || container._bkPaintGen !== gen;
  }
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    if (!container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        ...
        '</div>' +
        '<div class="bk-board-col">' +
        '<div class="bk-board-host" data-board></div>' +
        '</div></div>' +
        '<footer class="bk-footer" data-footer></footer></div>';
    }
    ...
    if (!header) return;
    header.innerHTML =
      '<span class="bk-title">Bank oder Risiko</span>' +
      '<span class="bk-badge">' + esc(PHASE_LABEL[game.phase] || game.phase || '') + '</span>' +
      (isShooter
        ? '<span class="bk-meta">Stand ' + esc(focusRange) +
    ...
    if (isShooter && me && game.phase === 'playing') {
      ...
      if (actions) {
        ...
        const btn = actions.querySelector('[data-bank]');
        if (btn) btn.onclick = function () { postControl('bank', { rangeNum: focusRange }); };
      }
    }

    const footer = container.querySelector('[data-footer]');
    if (footer) {
      footer.innerHTML = '<span class="bk-meta">Pot wächst mit dem Wert · unter dem Boden ist der Pot weg · Hole-in-Hole über 8,5 verdoppelt</span>';
    }
    if (container.id === 'shared-master-host') {
