...
func init() {
	loader.RegisterBuiltin("kettenreaktion", func(m *loader.Manifest) logicapi.Logic { return New(m) })
}
...
func (l *Logic) DefaultConfig() map[string]any {
	return map[string]any{
		"autoStartWhenAllReady": true, "shotsPerPlayer": 30, "chainBonus": 0.5, "chainMax": 5.0,
		...
		"handicaps": map[string]any{}, "chainProtections": map[string]any{},
	}
}
func (l *Logic) ConfigSchema() map[string]any {
	return map[string]any{"type": "object", "properties": map[string]any{"shotsPerPlayer": map[string]any{"type": "integer"}}}
}

type Player struct {
	RangeNum, ShotsFired, ChainLen, Longest, ProtectLeft int
	Active, Seated, Ready, WasWarmup, HasLast            bool
	ShooterName, Discipline, Color, Hint, LastNote       string
	Handicap, LastRaw, Score, Multiplier                 float64
	LastX, LastY                                         int
}
...
func (l *Logic) Init(cfg map[string]any) (logicapi.SessionState, error) {
	...
	gs := &GameState{Phase: gameutil.PhaseWarmup, NumRanges: gameutil.CfgInt(merged, "numRanges", 6), Config: merged, Players: map[string]*Player{}, StatusLine: "Kettenreaktion"}
	gs.ensure()
...
func (l *Logic) OnShotCtx(sess logicapi.SessionState, ctx logicapi.ShotContext) (logicapi.SessionState, []logicapi.PluginEvent, error) {
	...
	if !hih {
		if p.ChainLen > 0 && p.ProtectLeft > 0 {
			p.ProtectLeft--
		} else if raw > minVal {
			p.ChainLen = 1
		} else {
	...
	if p.ChainLen > p.Longest {
		p.Longest = p.ChainLen
	}
	n := p.ChainLen - 1
	if n < 0 {
		n = 0
	}
	mult := 1 + gameutil.CfgFloat(gs.Config, "chainBonus", 0.5)*float64(n)
	if cap := gameutil.CfgFloat(gs.Config, "chainMax", 5); mult > cap {
		mult = cap
	}
	p.Multiplier = mult
	p.Score += gameutil.Effective(raw, p.Handicap) * mult
	p.LastRaw, p.LastX, p.LastY, p.HasLast = raw, ctx.Shot.X, ctx.Shot.Y, true
	p.ShotsFired++
	note := fmt.Sprintf("×%.1f", mult)
	if hih {
		note = "Hole-in-Hole " + note
	}
	p.Hint, p.LastNote = fmt.Sprintf("Kette %d", p.ChainLen), note
	gs.pushShot(ShotMark{RangeNum: p.RangeNum, Raw: raw, Result: "score", Note: note, X: ctx.Shot.X, Y: ctx.Shot.Y, Color: p.Color})
...
func (l *Logic) ViewModel(sess logicapi.SessionState, rangeNum int) (map[string]any, error) {
	...
	for i := 1; i <= gs.NumRanges; i++ {
		...
		players = append(players, map[string]any{
			...
			"chainLen": p.ChainLen, "multiplier": p.Multiplier, "score": p.Score,
		})
	}
...
func (gs *GameState) beginPlay() []logicapi.PluginEvent {
	...
	gs.StatusLine = "Kette bauen — Hole-in-Hole über 8,5"
	prot := gameutil.CfgFloatMap(gs.Config, "chainProtections")
	for _, p := range gs.Players {
		if p == nil {
			continue
		}
		p.Seated, p.Score, p.ChainLen, p.Longest, p.Multiplier, p.HasLast, p.ShotsFired = false, 0, 0, 0, 1, false, 0
		p.Handicap = gameutil.HandicapFor(gs.Config, p.RangeNum)
		p.ProtectLeft = int(prot[gameutil.Itoa(p.RangeNum)])
	}
...
func (gs *GameState) maybeFinish() {
	...
	best, bestScore, bestChain := 0, -1.0, -1
	for _, p := range seated {
		if p.Score > bestScore || (p.Score == bestScore && p.Longest > bestChain) {
			best, bestScore, bestChain = p.RangeNum, p.Score, p.Longest
		}
	}
