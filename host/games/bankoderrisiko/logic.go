...
func init() {
	loader.RegisterBuiltin("bank-oder-risiko", func(m *loader.Manifest) logicapi.Logic { return New(m) })
}
...
func (l *Logic) DefaultConfig() map[string]any {
	return map[string]any{
		"autoStartWhenAllReady": true, "calibrateShots": 5, "floorOffset": 1.0, "shotsPerPlayer": 30,
...
func (l *Logic) ConfigSchema() map[string]any {
	return map[string]any{"type": "object", "properties": map[string]any{
		"calibrateShots": map[string]any{"type": "integer"}, "shotsPerPlayer": map[string]any{"type": "integer"},
		"floorOffset": map[string]any{"type": "number"}, "holeInHoleMultiplier": map[string]any{"type": "number"},
	}}
}
...
type Player struct {
	...
	HasLast, Calibrated                  bool
	ShooterName, Discipline, Color, Hint string
	LastNote                             string
	Handicap, LastRaw, Par, Floor        float64
	Pot, Banked                          float64
	LastX, LastY                         int
	CalValues                            []float64
}
...
func (l *Logic) Init(cfg map[string]any) (logicapi.SessionState, error) {
	...
	gs := &GameState{Phase: gameutil.PhaseWarmup, NumRanges: gameutil.CfgInt(merged, "numRanges", 6), Config: merged, Players: map[string]*Player{}, StatusLine: "Bank oder Risiko"}
	gs.ensure()
...
func (l *Logic) OnShotCtx(sess logicapi.SessionState, ctx logicapi.ShotContext) (logicapi.SessionState, []logicapi.PluginEvent, error) {
	...
	if raw < p.Floor {
		p.Pot = 0
		note, result = "Pot weg", "wipe"
	} else {
		p.Pot += eff
		note = fmt.Sprintf("+%.1f", eff)
		if hih {
			mult := gameutil.CfgFloat(gs.Config, "holeInHoleMultiplier", 2)
			p.Pot *= mult
			note = fmt.Sprintf("Hole-in-Hole ×%.1f", mult)
			result = "hole_in_hole"
		}
	}
	...
	p.ShotsFired++
	p.LastNote = note
	p.Hint = fmt.Sprintf("Pot %.1f · Bank %.1f", p.Pot, p.Banked)
...
func (l *Logic) Control(sess logicapi.SessionState, action string, params map[string]any) (logicapi.SessionState, []logicapi.PluginEvent, error) {
	...
	case "bank":
		...
		if p := gs.Players[gameutil.Itoa(rn)]; p != nil && p.Seated && gs.Phase == gameutil.PhasePlaying {
			p.Banked += p.Pot
			p.Pot = 0
			p.Hint = fmt.Sprintf("Pot 0 · Bank %.1f", p.Banked)
			gs.StatusLine = gameutil.PlayerLabel(p.ShooterName, p.RangeNum) + " bankt"
			events = append(events, logicapi.PluginEvent{Type: "banked", Data: map[string]any{"rangeNum": p.RangeNum, "banked": p.Banked}})
		}
	case "reset":
...
func (l *Logic) ViewModel(sess logicapi.SessionState, rangeNum int) (map[string]any, error) {
	...
	for i := 1; i <= gs.NumRanges; i++ {
		...
		players = append(players, map[string]any{
			...
			"floor": p.Floor, "pot": p.Pot, "banked": p.Banked,
		})
	}
...
func (gs *GameState) beginPlay() []logicapi.PluginEvent {
	...
	gs.StatusLine = "Bank oder Risiko — banken bevor der Pot kippt"
	off := gameutil.CfgFloat(gs.Config, "floorOffset", 1)
	for _, p := range gs.Players {
		if p == nil {
			continue
		}
		p.Seated, p.ShotsFired, p.HasLast, p.Pot, p.Banked = false, 0, false, 0, 0
		p.Handicap = gameutil.HandicapFor(gs.Config, p.RangeNum)
		p.Par = gs.parFor(p)
		p.Floor = p.Par - off
		p.Hint = fmt.Sprintf("Pot 0 · Boden %.1f", p.Floor)
	}
...
func (gs *GameState) maybeFinish() {
	...
	best, bestBank := 0, -1.0
	for _, p := range seated {
		if p.Banked > bestBank {
			best, bestBank = p.RangeNum, p.Banked
		}
	}
