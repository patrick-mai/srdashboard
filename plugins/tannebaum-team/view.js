...
(function () {
  ...
  function teamPickHtml(tree, me, focusRange) {
    if (!tree || tree.gameMode !== 'team' || !tree.canPickTeam || !focusRange) return '';
    const team = (me && me.team) || '';
    return '<div class="tb-panel tb-team-pick" data-team-pick>' +
      '<h3>Dein Team</h3>' +
      '<div class="tb-team-btns">' +
      '<button type="button" class="tb-team-btn' + (team === 'a' ? ' is-active' : '') +
      '" data-team="a">Team A</button>' +
      '<button type="button" class="tb-team-btn' + (team === 'b' ? ' is-active' : '') +
      '" data-team="b">Team B</button>' +
      '</div>' +
      '<div class="tb-muted">Stand ' + esc(focusRange) +
      (team ? ' · aktuell Team ' + esc(String(team).toUpperCase()) : ' · Standard: ungerade=A, gerade=B') +
      '</div></div>';
  }

  async function postSetTeam(rangeNum, team) {
    const body = JSON.stringify({
      action: 'set_team',
      params: { rangeNum: rangeNum, team: team }
    });
    const auth = window.SRAuth;
    const opts = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: body };
    let res;
    if (auth && typeof auth.fetchWithAuth === 'function') {
      res = await auth.fetchWithAuth('/api/plugins/control', opts);
    } else {
      res = await fetch('/api/plugins/control', opts);
    }
    if (!res.ok) {
      const t = await res.text();
      alert('Team-Wahl fehlgeschlagen: ' + t);
    }
  }

  function bindTeamPick(host, focusRange) {
    if (!host) return;
    host.dataset.rangeNum = String(focusRange || '');
    ...
    host.addEventListener('click', function (ev) {
      ...
      const rn = Number(host.dataset.rangeNum);
      if (!team || !rn) return;
      postSetTeam(rn, team);
    });
  }
  ...
  function renderView(pluginId) {
    return async function (container, viewModel, assetsBase) {
      ...
      container.querySelector('[data-header]').innerHTML =
        ...
        (me && me.team ? ' · Team ' + esc(String(me.team).toUpperCase()) : '') +
        '</span>' +
        '<span class="tb-status">' + esc(tree.statusLine || '') + '</span>';
      const pickHost = container.querySelector('[data-teampick]');
      if (pickHost) {
        pickHost.innerHTML = teamPickHtml(tree, me, focusRange);
        bindTeamPick(pickHost, focusRange);
      }
      container.querySelector('[data-hud]').innerHTML = hudHtml(tree) +
        (tree.startBlockedReason ? '<div class="tb-block">' + esc(tree.startBlockedReason) + '</div>' : '');
      ...
      container.querySelector('[data-footer]').innerHTML =
        ...
        ' · Fokus Stufe ' + esc((myTree && myTree.currentStage) || '—') +
        (me && me.team ? ' · Team ' + esc(String(me.team).toUpperCase()) : '') + '</span>';
    };
  }

  window.SRPluginViews['tannebaum-team'] = renderView('tannebaum-team');
})();
