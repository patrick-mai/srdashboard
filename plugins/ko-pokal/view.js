...
(function () {
    ...
    if (host.dataset.profile !== profileId || !host.querySelector('svg')) {
      ...
      try {
        ...
        if (svg) {
          ...
          svg.classList.add('kp-scheibe-svg');
        }
      } catch (e) {
        host.innerHTML = '<div class="kp-muted">Scheibe nicht ladbar</div>';
        return;
      }
    }
    const svg = host.querySelector('svg');
    if (!svg) return;
    let g = svg.querySelector('#kp-shots');
    if (!g) {
      g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('id', 'kp-shots');
      svg.appendChild(g);
    }
  ...
  function recentHtml(game, focusRange) {
    ...
    return '<div class="kp-panel"><h3>Letzte Schüsse</h3><ul class="kp-recent">' + list.map(function (s) {
      const me = focusRange && Number(s.rangeNum) === Number(focusRange);
      return '<li class="' + (me ? 'is-me' : '') + '"><span class="kp-swatch" style="background:' +
  ...
  function standingsHtml(game, focusRange) {
    ...
    return '<ul class="kp-standings">' + list.map(function (p) {
  ...
  function shotBlock(title, shot) {
    if (!shot) return '<div class="kp-panel"><h3>' + esc(title) + '</h3><div class="kp-muted">—</div></div>';
    return '<div class="kp-panel"><h3>' + esc(title) + '</h3><div class="kp-shot-val">' +
      fmt1(shot.raw) + '</div><div class="kp-meta">' + esc(shot.note || '') + '</div></div>';
  }
  ...
  function boardSvg(game, focusRange) {
    const matches = game.bracket || [];
    const cur = game.currentMatch || {};
    let html = '<div class="kp-panel"><h3>Live</h3><div class="kp-shot-val">' +
      esc(cur.leftLabel || '—') + ' ' + esc(cur.leftPoints || 0) + ' : ' +
      esc(cur.rightPoints || 0) + ' ' + esc(cur.rightLabel || '—') + '</div></div>';
    matches.forEach(function (m) {
      html += '<div class="kp-panel"><h3>' + esc(m.roundLabel || 'Runde') + '</h3>' +
        esc(m.leftLabel || '') + ' vs ' + esc(m.rightLabel || '') +
        (m.winnerLabel ? ' → ' + esc(m.winnerLabel) : '') + '</div>';
    });
    return html;
  }

  function setKpSurfaceClasses(container, mode) {
    if (!container) return;
    container.classList.add('range-plugin-view', 'kp-view');
    if (container.id === 'shared-master-host') {
      container.classList.add('shared-master-host');
    }
    if (mode === 'master') {
      container.classList.add('kp-race-master');
      container.classList.remove('kp-race-shooter');
    } else {
      container.classList.add('kp-race-shooter');
      container.classList.remove('kp-race-master');
    }
  }

  function beginPaint(container) {
    const gen = (container._kpPaintGen || 0) + 1;
    container._kpPaintGen = gen;
    return gen;
  }

  function kpPaintStale(container, gen) {
    return !container || container._kpPaintGen !== gen;
  }
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    setKpSurfaceClasses(container, isMaster ? 'master' : 'shooter');

    const layoutCls = isMaster ? 'kp-master-layout' : 'kp-shooter-layout';
    const otherCls = isMaster ? 'kp-shooter-layout' : 'kp-master-layout';
    if (container.querySelector('.' + otherCls) || !container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        '<div class="' + layoutCls + '">' +
        '<header class="kp-header" data-header></header>' +
        '<div class="kp-main">' +
        '<div class="kp-target-col">' +
        '<div class="kp-scheibe-wrap" data-scheibe></div>' +
        (isMaster ? '' : '<div class="kp-shot-hud" data-shothud></div>') +
        '<div data-recent></div>' +
        '<div data-standings></div>' +
        '<div data-actions></div>' +
        '</div>' +
        '<div class="kp-board-col">' +
        '<div class="kp-board-host" data-board></div>' +
        '</div></div>' +
        '<footer class="kp-footer" data-footer></footer></div>';
    }

    await ensureTargetRegistry(assetsBase);
    if (kpPaintStale(container, paintGen)) return;
    const core = window.SRCore;
    ...
    if (!header) return;
    header.innerHTML =
      '<span class="kp-title">KO-Pokal</span>' +
      '<span class="kp-badge">' + esc(PHASE_LABEL[game.phase] || game.phase || '') + '</span>' +
      (isShooter
        ? '<span class="kp-meta">Stand ' + esc(focusRange) +
          (me && me.shooterName ? ' · ' + esc(me.shooterName) : '') + '</span>'
        : '') +
      '<span class="kp-status">' + esc(game.statusLine || '') + '</span>';
    ...
    container.querySelector('[data-standings]').innerHTML =
      '<div class="kp-panel"><h3>Stand</h3>' + standingsHtml(game, isMaster ? null : focusRange) + '</div>';
    container.querySelector('[data-board]').innerHTML =
      (game.startBlockedReason ? '<div class="kp-block">' + esc(game.startBlockedReason) + '</div>' : '') +
      boardSvg(game, isMaster ? null : focusRange);

    const footer = container.querySelector('[data-footer]');
    if (footer) {
      footer.innerHTML = '<span class="kp-meta">Quali setzt die Setzliste · Duell Punkt für Punkt · Hole-in-Hole über 8,5 stiehlt den Punkt</span>';
    }
    if (container.id === 'shared-master-host') {
