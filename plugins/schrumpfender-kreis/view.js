...
(function () {
  ...
  async function render(container, viewModel, assetsBase) {
    ...
    setSkSurfaceClasses(container, isShooter ? 'shooter' : 'master');

    const layoutCls = isShooter ? 'sk-shooter-layout' : (isCompact ? 'sk-compact-layout' : 'sk-master-layout');
    if (!container.querySelector('.' + layoutCls)) {
      container.innerHTML =
        ...
        (isCompact ? '' : '<div class="sk-scheibe-wrap" data-scheibe></div>') +
        (isShooter ? '<div class="sk-shot-hud" data-shothud></div>' : '') +
        '<div data-recent></div>' +
    ...
    container.querySelector('[data-standings]').innerHTML =
      '<div class="sk-panel"><h3>Stand</h3>' + standingsHtml(game, isShooter ? focusRange : null) + '</div>';
